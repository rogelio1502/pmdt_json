...
$githubUrl = "https://raw.githubusercontent.com/LuigiEspinosa/promedica-scraper/main/json/ProMedica/agency/home-health-details.json";

$jsonInfo = curl $githubUrl -UseBasicParsing | ConvertFrom-Json
$newLocationsPath = $db + "/NewLocations/HomeHealth"
...
foreach($row in $jsonInfo){
    ...
    foreach($ltn in $newLocationsFolder.Children){
        ...
        if($directUrl -eq $importDirectUrl){
            ...
            $_item = $cardItemN
            $_item.Editing.BeginEdit()
            $_item['Title'] = "Our Team"
            $_item["Description"] = $row.content.ourTeam.content
            $_item.CardImage = Get-Item -Path "master:\media library\pmdt-jss-site\data\media\img\_placeholder_370x247"
            [Sitecore.Data.Fields.LinkField] $linkField = $_item.Fields["Url"];
            $linkField.Clear()
            $linkField.Text = "Learn More";
            $linkField.LinkType = "external";
            $linkField.Url = $row.content.ourTeam.leanrmore;
            $_item.Editing.EndEdit()
            ...
            $hasBlock = 0
            foreach($block in $ltn.Children['FeaturedContentBlocks'].Children){
                $hasBlock = 1
                $newTextContentBlockItemPath = $ltn.Children['FeaturedContentBlocks'].Paths.Path + "/FeaturedContentBlock1"
                $newTextContentBlockItem = Get-Item -Path $newTextContentBlockItemPath
                $_item = $newTextContentBlockItem
                $_item.Editing.BeginEdit()
                $_item['Title'] = "Enriching Life"
                $_item['Description'] = $row.content.enrichingLife.content
                [Sitecore.Data.Fields.LinkField] $linkField = $_item.Fields["CTA"];
                $linkField.Clear()
                $linkField.Text = "";
                $linkField.LinkType = "external";
                $linkField.Url = $row.content.enrichingLife.video;
            ...
            if($hasBlock -eq 0){
                ...
                $_item['Title'] = "Enriching Life"
                $_item['Description'] = $row.content.enrichingLife.content
                $_path = $mediaFolder + $row.content.enrichingLife.imgSrc.Split('/')[-1].Split('.')[0]
                $_item.BlockImage = Get-Item -Path $path
                [Sitecore.Data.Fields.LinkField] $linkField = $_item.Fields["CTA"];
                $linkField.Clear()
                $linkField.Text = "";
                $linkField.LinkType = "external";
